...
**Backend**
- `collect_restaurants.py`: 수집 방식 개선
  - `bukgu` 격자: step 0.8km → **0.5km** (격자 2.5배 증가, 누락 최소화)
  - 페이지: 3 → **4** (격자 포인트당 최대 60개 → 45개)
  - `bukgu_dense` 신규 추가: 용봉·중흥·운암·일곡 핵심 상권 0.3km 초밀도 수집
  - CLI 옵션에 `bukgu_dense` 추가
- `admin.py`: 수집 셀렉트에 "북구 핵심 상권 (0.3km 초밀도)" 옵션 추가

> **참고**: 카카오 수집은 키워드 없이 카테고리 코드(FD6=음식점/CE7=카페)로 수집.  
> 술집·호프·이자카야도 FD6에 포함되어 별도 키워드 불필요.  
> 키워드 방식은 네이버 보강(supplementary)에서만 사용.

---

## [1.4.5] — 2026-05-21

### 재배포 시 로그아웃 버그 수정 + 술집 수집 확대

**Flutter**
- `api_service.dart` (worktree + hankkirog): `validateToken()` 내부 try-catch 제거
  - 기존: 네트워크 오류 → `false` 반환 → Railway 재배포 중 다운타임에 자동 로그아웃
  - 수정: 예외 그대로 전파 → `_checkAuth()`의 catch가 처리 → 로그아웃 방지

**Backend**
- `enrich_naver.py`: 수집 키워드 29→43개 확대
  - 술집·주점 키워드 추가: 호프·술집·주점·이자카야·포장마차·막걸리·와인바
  - 음식 키워드 추가: 쌀국수·돈가스·덮밥·쌈밥
  - 동네 키워드 추가: 임동·신안동·우산동·동림동
  - `_ALCOHOL_KW` 확장: 술집·주점·포장마차·막걸리·와인·소주·안주·펍·바 추가
